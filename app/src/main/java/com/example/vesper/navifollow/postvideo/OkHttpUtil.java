package com.example.vesper.navifollow.postvideo;
...
//    }
    ...
    public static void postFile(String url, final ProgressListener listener, Callback callback, File...files){
        ...
        final MediaType MEDIA_TYPE_TXT = MediaType.parse("txt/plain");
        final File file2=new File(Environment.getExternalStorageDirectory().getPath()+"/mahc/video/temp/steps.txt");
        final File file3=new File(Environment.getExternalStorageDirectory().getPath()+"/mahc/video/temp/orients.txt");
        builder.setType(MultipartBody.FORM);
        Log.i("huang","files[0].getName()=="+files[0].getName());
        builder.addFormDataPart("tablename", PostVideo.tablename);
        ...
        builder.addFormDataPart("file2","steps.txt",RequestBody.create(MEDIA_TYPE_TXT,file2));
        builder.addFormDataPart("file3","orients.txt",RequestBody.create(MEDIA_TYPE_TXT,file3));
