...
public class StepSensorAcceleration extends StepSenorBase{
...
//        } else if (CountTimeState == 2) {
        StepSenorBase.CURRENT_SETP++;
//            if (stepCallBack != null) {
        stepCallBack.Step(StepSenorBase.CURRENT_SETP);
//            }
//        }
...
//            Log.v(TAG, "else");
    ...
    class TimeCount extends CountDownTimer {
        ...
        public void onFinish() {
            // 如果计时器正常结束，则开始计步
            time.cancel();
            StepSenorBase.CURRENT_SETP += TEMP_STEP;
            lastStep = -1;
            Log.v(TAG, "计时正常结束");
            ...
            TimerTask task = new TimerTask() {
                public void run() {
                    if (lastStep == StepSenorBase.CURRENT_SETP) {
                        timer.cancel();
                        CountTimeState = 0;
                        lastStep = -1;
                        TEMP_STEP = 0;
                        Log.v(TAG, "停止计步：" + StepSenorBase.CURRENT_SETP);
                    } else {
                        lastStep = StepSenorBase.CURRENT_SETP;
                    }
                }
            };
